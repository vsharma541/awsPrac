...
# MyNotes
This application is used to create a lambda function which has an API gateway
This API gateway also has logging enabled, means it will log the API gw logs as well
Refer - https://www.serverless.com/framework/docs/providers/aws/events/apigateway for API gw logging configuration using serverless
In serverless.yml, the API gw is enabled as follows:
`````
provider:
  name: aws
  runtime: nodejs20.x
  logs:
    restApi: true
``````
By default, both access logging and execution logging are enabled
